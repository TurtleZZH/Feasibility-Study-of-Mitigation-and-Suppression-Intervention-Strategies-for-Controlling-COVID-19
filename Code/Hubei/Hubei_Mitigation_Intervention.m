...
for idx = 1:length(T)-1
    r2(idx+1) = r2(idx);
    if idx>=32                                                             %以十二月二十一日为起准，32日后即一月二十三日进行限制流通措施 % As of December 21st, restrictions on circulation will be implemented on January 23 after 32 days
        if r2(idx) ~= 8                                                    %强干预下，流通限制从15到8，采取限制后每天减少1.75个人 Under strong intervention, the circulation limit is from 15 to 8. After the restriction is adopted, 1.75 people will be reduced every day
            r2(idx+1) = r2(idx)-1.75;
        end
    end
     %易感者 susceptible
...
hold on;
title('Hubei Mitigation Intervention SEIR model')