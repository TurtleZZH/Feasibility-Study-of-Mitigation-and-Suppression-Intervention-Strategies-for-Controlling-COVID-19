...
%--------------------------------------------------------------------------
clear;clc;
%---------------------London Suppression Intervention----------------------
%--------------------------------------------------------------------------
%   参数设置
%--------------------------------------------------------------------------
...
for idx = 1:length(T)-1
    r2(idx+1) = r2(idx);
    if idx>=35                                                             %以二月七日为起准，35日后即三月一十二日进行限制流通措施 With effect from February 7th, 35 days later, on March 12th, restrictions on circulation will be implemented
        if r2(idx) ~= 3                                                    %强干预下，流通限制从15到3，采取限制后每天减少三个人 Under suppression intervention,  the circulation limit is from 15 to 3, and three people are reduced every day after taking the restriction
            r2(idx+1) = r2(idx)-3;
        end
    end
    %易感者 susceptible
...
hold on;
title('London Suppression Intervention SEIR model')